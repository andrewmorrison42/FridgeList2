...
const MUTATIONS = [
  ...
  { name: 'waitlist-fulfilled-by-clock',
    rule: '"Added before the close" is causal, never by timestamp (P8, §5.3)',
    file: 'src/core/generate.js',
    find: '&& reg.by.some((added) => closeEvents.some((closed) => happenedBefore(added, closed))));',
    replace: '&& reg.by.some((added) => closeEvents.some((closed) => added.ts < closed.ts)));' },
  { name: 'waitlist-fulfilled-by-any-close',
    rule: 'A tick in a shop does not fulfil an item added after that shop closed (P8)',
    file: 'src/core/generate.js',
    find: '&& reg.by.some((added) => closeEvents.some((closed) => happenedBefore(added, closed))));',
    replace: ');' },
