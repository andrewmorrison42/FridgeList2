...
const MUTATIONS = [
  ...
  { name: 'carry-counter',
    rule: 'Carry-over is set membership, never a counter (§9.1)',
    file: 'src/core/keys.js',
    find: "case 'menu.carried':        return K.menuCarried(p.recipeId, p.shopId);",
    replace: "case 'menu.carried':        return K.menuCarried(p.recipeId, `${p.shopId}-${event.dev}`);" },
